  ...
  )
{
  ...
  while ((Link != &RequestBlockArray->Entry) && (Link->ForwardLink != &RequestBlockArray->Entry)) {
    ...
    if ((NextBlockData->Offset >= BlockData->Offset) && (NextBlockData->Offset - BlockData->Offset <= BlockData->Width)) {
      if (NextBlockData->Offset - BlockData->Offset + NextBlockData->Width > BlockData->Width) {
        BlockData->Width = (UINT16)(NextBlockData->Offset - BlockData->Offset + NextBlockData->Width);
      }
  ...
  )
{
  ...
  for (Link = BlockLink->ForwardLink; Link != BlockLink; Link = Link->ForwardLink) {
    ...
    for (TempLink = Link->ForwardLink; TempLink != BlockLink; TempLink = TempLink->ForwardLink) {
      ...
      if (!NextBlockData->IsBitVar || (NextBlockData->Offset >= BlockData->Offset && NextBlockData->Offset - BlockData->Offset >= BlockData->Width) || (BlockData->Offset >= NextBlockData->Offset && BlockData->Offset - NextBlockData->Offset >= NextBlockData->Width)) {
        continue;
      }
